...
def add_model_options(parser):
    ...
    group.add_argument("--latent_dim", default=512, type=int, help="dimensionality of the latent space (512 for CLIP compatibility)")
    ...
    # Enhanced motion-preserving losses
    group.add_argument("--lambda_range", default=0.0, type=float, help="weight of the range preservation loss")
    group.add_argument("--lambda_veldist", default=0.0, type=float, help="weight of the velocity distribution loss")
    group.add_argument("--lambda_diversity", default=0.0, type=float, help="weight of the motion diversity loss")
    group.add_argument("--lambda_spatial", default=0.0, type=float, help="weight of the spatial coherence loss")
    
    # Dual forward pass: train decoder on CLIP text features
    group.add_argument("--lambda_textrecon", default=0.0, type=float, help="weight of text-to-motion reconstruction loss (dual forward pass)")
    ...
    group.add_argument("--clip_lambda_visual", default=0.0, type=float, help="weight of the Visual CLIP loss (rendered point tracks), if > 0, enables visual grounding.")
    ...
    group.add_argument('--temporal_encoding', dest='temporal_encoding', action='store_true', 
                      help="Use temporal encoding (preserve sequence structure in encoder)")
    group.set_defaults(temporal_encoding=False)
    
    # NEW: MLP decoder (from memory: best for motion preservation)
    group.add_argument('--mlp_decoder', dest='mlp_decoder', action='store_true',
                      help="Use MLP decoder instead of Transformer (no motion compression)")
    group.set_defaults(mlp_decoder=False)
    
    # NEW: Cross-attention to 77 CLIP tokens
    group.add_argument('--use_cross_attention', dest='use_cross_attention', action='store_true',
                      help="Use cross-attention to 77 CLIP tokens in decoder")
    group.set_defaults(use_cross_attention=False)
